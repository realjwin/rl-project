...
def init_scenario(num_users, xlim, ylim, epsilon_distance, noise_power,
    ...
    # Initialize user powers randomly
    user_power_idx = np.random.randint(0, len(p_valid), num_users)
...
def set_locations(scenario):
    ...
    for user_idx in range(num_users):
        user_locations[user_idx] = np.asarray([np.random.randint(low=np.min(xlim), high=np.max(xlim)),
                                               np.random.randint(low=np.min(ylim), high=np.max(ylim))])
